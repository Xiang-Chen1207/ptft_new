...
def get_config(model_type):
    config = {
        'model': {
            ...
            'd_model': 200,
            'dim_feedforward': 800,
            # CHANGED: seq_len 1 for TUEV (1s patch)
            'seq_len': 1,
            'n_layer': 12,
            'nhead': 8,
            'dropout': 0.1,
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='experiments/tuev_lp/features')
    ...
    # CHANGED: Use TUEV logic
    train_files = get_tuev_file_list(args.dataset_dir, 'train', seed=42)
    val_files = get_tuev_file_list(args.dataset_dir, 'val', seed=42)
    test_files = get_tuev_file_list(args.dataset_dir, 'test', seed=42)
    ...
    train_dataset = TUEVDataset(train_val_files)
    test_dataset = TUEVDataset(test_files)
